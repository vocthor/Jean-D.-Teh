...
let rec trad_instructions instructions oc =
  ...
  | Copy n :: rest      -> Printf.fprintf oc "\t\tlet nth = self.stack[%d-1]; self.stack.push(nth); // copy\n" n ; trad_instructions rest oc
  ...
  | OutputChar :: rest  -> Printf.fprintf oc "\t\tprint!(\"{}\", self.stack.pop().unwrap() as u8 as char); // output char\n" ; trad_instructions rest oc
  | OutputNum :: rest   -> Printf.fprintf oc "\t\tprint!(\"{}\", self.stack.pop().unwrap()); // output num\n" ; trad_instructions rest oc
  | ReadChar :: rest    -> Printf.fprintf oc "\t\tlet mut input = String::new(); io::stdin().read_line(&mut input).expect(\"Error: unable to read user char input\"); let addr: i32 = self.stack.pop().unwrap(); self.memory.insert(addr,input.chars().nth(0).unwrap() as i32); // read char\n" ; trad_instructions rest oc
  | ReadNum :: rest     -> Printf.fprintf oc "\t\tlet mut input = String::new(); io::stdin().read_line(&mut input).expect(\"Error: unable to read user char input\"); let addr: i32 = self.stack.pop().unwrap(); input.pop(); self.memory.insert(addr, input.parse::<i32>().unwrap()); // read num\n" ; trad_instructions rest oc